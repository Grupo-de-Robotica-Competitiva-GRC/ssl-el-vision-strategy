...
{
    Q_ASSERT(id == 0); // Apenas uma câmera

    // Definir a posição da câmera para centralizar no campo
    QVector<QPair<float, float>> camera_position = {
        {0.0, 0.0} // Câmera centralizada
    };

    return {camera_position[id].first * (float)cfg->Field_Length(),
            camera_position[id].second * (float)cfg->Field_Width()};
}
...
SSL_WrapperPacket *SSLWorld::generatePacket()
{
    ...
    // Defina um ID de câmera fixo para a câmera única
    const int cam_id = 0;
    ...
    if (((sendGeomCount++) / 4) % (cfg->sendGeometryEvery() / 4) == 0)
    {
        ...
        // Adicionando informações do campo
        ...
        addFieldLinesArcs(field); // Adicionar linhas e arcos do campo

        // Câmera única - calibração
        SSL_GeometryCameraCalibration *camera = geom->add_calib();
        camera->set_camera_id(cam_id);
        camera->set_distortion(0.2);
        camera->set_focal_length((float)cfg->Camera_Focal_Length());
    ...
    // Verificação de visibilidade da bola
    if (visibleInCam(0, x, y)) // Mude aqui para usar a nova lógica
    {
        ...
        float output_y = randn_notrig(y * 1000.0f, dev_y);
    ...
    // Verifica a visibilidade dos robôs
    for (int i = 0; i < cfg->Robots_Count() * 2; i++)
    {
        ...
        if (!cfg->vanishing() || (rand0_1() > vanishing))
        {
            ...
            // Reset quando o robô virou
            if (cfg->ResetTurnOver() && k < 0.9)
                robots[i]->resetRobot();

            // Verificação de visibilidade do robô
            if (visibleInCam(0, x, y)) // Mude aqui para usar a nova lógica
            {
...
void SSLWorld::sendVisionBuffer()
{
    ...
    // Gera apenas um pacote para a visão do campo inteiro
    SSL_WrapperPacket *packet = generatePacket(); // Usar uma câmera padrão
    sendQueue.push_back(new SendingPacket(packet, t));

    // Enquanto os pacotes na fila são mais antigos que o atraso de envio configurado
